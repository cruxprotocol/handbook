...
Writes to Gaia requires an authentication token which needs to be signed with the User owned private key. Any data written is always written along with an [HMAC](https://en.wikipedia.org/wiki/HMAC); also generated using the same private key. The Auth token helps us ensure write-time correctness and also sets others up to do read-time verification.
